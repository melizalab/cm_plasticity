...
from core import junction_potential, setup_log
...
if __name__ == "__main__":
    ...
    iv_stats = pd.read_csv(
        args.build_dir / "iv_stats.csv", index_col=["cell", "epoch", "sweep"]
    )
    sweep_stats = pd.read_csv(
        args.build_dir / "sweep_stats.csv", index_col=["cell", "epoch", "sweep"]
    )
    epoch_stats = pd.read_csv(
        args.build_dir / "epoch_stats.csv", index_col=["cell", "epoch"]
    )
    ...
    steps_binned = steps.groupby(
        ["epoch", pd.cut(steps.current, bins, labels=False)]
    ).mean()
    ...
    for enumber, epoch in steps_binned.groupby("epoch"):
        axes[0].plot(
            epoch.current, epoch.voltage, label="%d s" % int(epochs.loc[enumber].time)
        )
    ...
    for enumber, epoch in sweeps.groupby("epoch"):
        axes[1].plot(
            epoch.current,
            epoch.firing_rate,
            label="%d s" % int(epochs.loc[enumber].time),
        )
    ...
    axes[0].errorbar(
        epochs.time,
        epochs.duration_mean,
        epochs.duration_sd / np.sqrt(epochs.n_spike_sweeps),
        **marker_style,
    )
    ...
    axes[3].errorbar(
        epochs.time, epochs.Vm, epochs.Vm_sd / np.sqrt(epochs.n_sweeps), **marker_style
    )
    axes[3].set_ylim(epochs.Vm.mean() - 10, epochs.Vm.mean() + 10)
    axes[3].set_ylabel("Vm (mV)")
    axes[4].errorbar(
        epochs.time, epochs.Rm, epochs.Rm_sd / np.sqrt(epochs.n_sweeps), **marker_style
    )
    Rmm = epochs.Rm.mean()
    ...
    for eidx in [epoch_idx[0], epoch_idx[-1]]:
        ...
        for sidx in args.sweeps:
            ...
            V = (segment.analogsignals[0].load() - junction_potential).rescale("mV")
            I = segment.analogsignals[1].load().rescale("pA")
